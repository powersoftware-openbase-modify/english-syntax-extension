...
- **快捷键解析单段**：鼠标停在某段上按 `Alt+C` 只解析那一段，会话没启动也能直接按（自动轻量启动，不触发全文扫描）——长文档不必为了看一段付出上百次模型请求。两端都可改键：Chrome 在 `chrome://extensions/shortcuts`，IntelliJ 在 `Settings → Keymap`；
...
3. 或者不开整页学习：把鼠标悬停在某个段落上按 `Alt+C`（Mac：`Option+C`），只解析该段；快捷键可在 `chrome://extensions/shortcuts` 修改；
