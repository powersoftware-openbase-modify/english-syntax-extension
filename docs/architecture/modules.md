...
| `analysis-validator.ts`         | `validateCoreBatch()` / `validateDetail()`——模型输出的最终裁判：结构/覆盖率/安全校验，以及十三条本地语法粒度硬约束（连续相邻谓语、单词白名单介词、废弃的 `COORDINATE_CLAUSE`、至少含 FANBOYS 的连词、谓语首词是限定词/主格代词、谓语内部含限定词、从属连词引导的并列分句、单成分包住整句——`FRAGMENT_HEAD` 唯一豁免、单词从句、定语从句后紧跟其宾语、成分以必带宾语介词收尾、`FRAGMENT_HEAD` 至多一个、`FRAGMENT_HEAD` 不与分句级角色混用）；错误文案直接供 repair prompt 使用                        |
